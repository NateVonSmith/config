...
function forkJoin() {
    ...
    if (sources.length === 1) {
        var first = sources[0];
        if (isArray_1.isArray(first)) {
            return forkJoinInternal(first, null);
        }
        // TODO(benlesh): isObservable check will not be necessary when deprecated path is removed.
        if (isObject_1.isObject(first) && Object.getPrototypeOf(first) === Object.prototype) {
            var keys = Object.keys(first);
            return forkJoinInternal(keys.map(function (key) { return first[key]; }), keys);
        }
    }
    // DEPRECATED PATHS BELOW HERE
    if (typeof sources[sources.length - 1] === 'function') {
        var resultSelector = sources.pop(), as = Function;
        sources = (sources.length === 1 && isArray_1.isArray(sources[0])) ? sources[0] : sources;
        return forkJoinInternal(sources, null).pipe(map_1.map(function (args) { return resultSelector.apply(void 0, args); }));
    }
